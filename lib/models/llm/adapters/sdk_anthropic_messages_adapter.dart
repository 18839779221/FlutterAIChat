...
/// SDK-backed implementation of [ApiStyleAdapter] for the Anthropic Messages
/// protocol, powered by `anthropic_sdk_dart`.
///
/// This adapter uses the `anthropic_sdk_dart` package to build correct request
/// payloads and parse responses, avoiding manual JSON construction.
class SdkAnthropicMessagesAdapter extends ApiStyleAdapter {
  const SdkAnthropicMessagesAdapter();
  ...
  ProtocolRequestSpec buildChatRequestSpec({
    ...
    final request = _buildMessagesRequest(
      messages: messages,
      config: config,
      modelName: modelName,
      requestOptions: requestOptions,
    );
    return AnthropicMessagesRequestSpec(request: request);
  }
  ...
  anthropic.MessageCreateRequest _buildMessagesRequest({
    required List<ChatMessage> messages,
    required ChatConfig config,
    required String modelName,
    ...
    for (final message in messages) {
      ...
      normalizedMessages.add(_buildMessage(message));
    }
    ...
    return anthropic.MessageCreateRequest.fromJson(payload);
  }
  ...
  void _applyCacheHints(
    ...
    if (cache.strategy != LlmCacheStrategy.providerHints) return;
    if (!cache.markStableSystemPrefix) return;

    final systemValue = payload['system'];
    if (systemValue is! String || systemValue.trim().isEmpty) return;
  ...
  PlannerToolChoice? parsePlannerChoice(Map<String, dynamic> payload) {
    ...
    for (final item in content) {
      ...
      final response = _extractContentText(normalizedItem);
      if (response != null) {
  ...
  String extractNonStreamText(Map<String, dynamic> payload) {
    ...
    for (final item in content) {
      if (item is! Map) continue;
      final text = _extractContentText(item.cast<String, dynamic>());
  ...
  ModelTurnDecision? parseDecision(Map<String, dynamic> payload) {
    ...
    for (var i = 0; i < content.length; i++) {
      ...
      if (normalizedItem['type'] == 'tool_use') {
        ...
        if (toolName == null || input is! Map) continue;
        toolCalls.add(
          ModelToolCall(
      ...
      if (type == 'thinking' || type == 'redacted_thinking') {
        ...
        if (thinking != null) reasoningBuffer.write(thinking);
        continue;
      }
    ...
    final assistantMessage = normalizeAggregatedText(textBuffer.toString());
  ...
  String? _extractContentText(Map<String, dynamic> item) {
    final type = item['type'];
    if (type != 'text' && type != 'thinking' && type != 'redacted_thinking') {
      return null;
    }
    return normalizeText(item['text'] ?? item['thinking']);
  }
  ...
  ProtocolRequestSpec buildPlannerRequestSpecFromCarriers({
    ...
    return AnthropicMessagesRequestSpec(
      request: anthropic.MessageCreateRequest.fromJson(payload),
    );
  }
