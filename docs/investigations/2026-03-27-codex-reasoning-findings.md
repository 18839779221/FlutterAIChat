...
> 说明：本文记录的是 2026-03-27 当时仍存在“深度思考”请求入口时的排查结论。当前主线实现已移除该配置开关，think/reasoning 改为仅作为 provider 返回内容处理。

## 结论摘要

当时 Codex 这套 `baseUrl + model` 方案下，“深度思考”请求并不是完全没生效。
...
这说明当时页面上的“深度思考”开关在部分请求中是能真正传递到后端的。
...
当时那条 Codex 链路下，“没有 reasoning 可显示”的直接原因是：
...
3. 如果 provider 只能返回 `encrypted_content`，则 UI 只能展示最终答案，不能伪造可见 think 文本
